...
export default function ProyectoEditForm({
  ...
  return (
    <form action={dispatch} className="bg-white p-8 rounded-lg shadow-md grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
      {/* Column 1 */}
      <div className="flex flex-col gap-6">
        <div>
          ...
          <input type="text" name="titulo" id="titulo" required defaultValue={proyecto.titulo} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
        </div>
        <div>
          <label htmlFor="storyline" className="block text-sm font-medium text-primary mb-1">Storyline</label>
          <textarea name="storyline" id="storyline" rows={4} defaultValue={proyecto.storyline || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary"></textarea>
        </div>
        <div>
          <label htmlFor="id_categoria" className="block text-sm font-medium text-primary mb-1">Categoría</label>
          <select name="id_categoria" id="id_categoria" defaultValue={proyecto.id_categoria || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">
        ...
        <div>
          ...
          <select name="id_subcategoria" id="id_subcategoria" defaultValue={proyecto.id_subcategoria || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">
            <option value="">Seleccionar...</option>
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <select name="id_dependencia_origen" id="id_dependencia_origen" defaultValue={proyecto.id_dependencia_origen || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">
            <option value="">Seleccionar...</option>
        ...
        <div>
          ...
          <select name="id_dependencia_actual" id="id_dependencia_actual" defaultValue={proyecto.id_dependencia_actual || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">
            <option value="">Seleccionar...</option>
            {dependencias.map(d => <option key={d.id} value={d.id}>{d.nombre}</option>)}
          </select>
        </div>
        <div>
          ...
          <input type="url" name="url_ticketera_interna" id="url_ticketera_interna" placeholder="https://..." defaultValue={proyecto.url_ticketera_interna || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
        </div>
        <div>
          <label htmlFor="url_ticketera_externa" className="block text-sm font-medium text-primary mb-1">URL Ticketera Externa</label>
          <input type="url" name="url_ticketera_externa" id="url_ticketera_externa" placeholder="https://..." defaultValue={proyecto.url_ticketera_externa || ''} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
        </div>
        <div>
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
                ...
                <input type="number" name="tier" id="tier" min="1" max="5" defaultValue={proyecto.tier || ''} className="w-24 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
            </div>
            <div className="flex items-center">
              <input type="checkbox" name="activo" id="activo" defaultChecked={proyecto.activo || false} className="h-4 w-4 text-primary border-gray-300 rounded focus:ring-primary" />
      ...
      <div className="md:col-span-2 pt-6 border-t border-gray-300">
        ...
        {proyecto.captura_type ? (
          <div className="mb-4">
            ...
            <Image src={`/api/proyectos/${proyecto.id}/captura`} alt="Captura actual" width={300} height={200} className="rounded-md border" />
            <div className="flex items-center mt-2">
              <input type="checkbox" name="delete_captura" id="delete_captura" className="h-4 w-4 text-tag-red border-gray-300 rounded focus:ring-tag-red" />
        ...
        <div>
          ...
          <input type="file" name="captura" id="captura" accept="image/jpeg, image/png" className="w-full text-sm text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"/>
        </div>
      </div>

      <div className="md:col-span-2 flex justify-end items-center gap-4 mt-4">
        <Link href={`/proyectos/${proyecto.id}`} className="text-secondary hover:underline text-sm">Cancelar</Link>
        <SubmitButton />
      </div>
    </form>
  );
