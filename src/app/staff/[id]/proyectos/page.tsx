...
import StaffProyectosForm from '@/components/StaffProyectosForm';

export default async function StaffProyectosPage({ params }: { params: { id: string } }) {
  const id = parseInt(params.id, 10);
  const [staff, proyectos] = await Promise.all([
  ...
  return (
    <main className="flex flex-col items-center w-full min-h-screen p-8 bg-background">
      <div className="w-full max-w-6xl">
            ...
            {tabs.map((tab) => (
              <Link
                ...
                className={`border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm no-underline ${
                  tab.name === 'Proyectos' ? 'border-primary text-primary' : ''
                }`}
              >
                {tab.name}
              </Link>
        ...
        <div className="mt-8">
          <StaffProyectosForm staff={staff} proyectos={proyectos} />
        </div>
      </div>
    </main>
  );
