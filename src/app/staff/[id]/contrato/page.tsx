...
import StaffContratoForm from '@/components/StaffContratoForm';

export default async function StaffContratoPage({ params }: { params: { id: string } }) {
            ...
            {tabs.map((tab) => (
              <Link
                ...
                className={`border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm no-underline ${
                  tab.name === 'Contrato' ? 'border-primary text-primary' : ''
                }`}
              >
                {tab.name}
              </Link>
